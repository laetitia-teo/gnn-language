...
class ACModelGNNDense(nn.Module, babyai.rl.RecurrentACModel):
    def __init__(self, obs_space, action_space,
                 image_dim=5, memory_dim=(4, 8), instr_dim=128, nheads=1,
                 use_instr=False, lang_model="gru", use_memory=False, arch="cnn1",
                 aux_info=None):
        ...
        self.obs_space = obs_space
        self.slot_memory_model = SlotMem(
            K=self.memory_dim[0],
            Fin=self.image_dim,
            Fmem=self.memory_dim[1],
            nheads=nheads
    ...
    def forward(self, obs, memory, obs_batch, m_batch, instr_embedding=None):
        ...
        t0 = time.time()
        output, memory, log_time_slot_memory_model = self.slot_memory_model(obs, memory)
        t_slot_memory_model = time.time() - t0

        t0 = time.time()
        embedding = scatter_sum(output, m_batch.type(torch.LongTensor))
        # TODO: scatter sum or get first slot of memory ?
